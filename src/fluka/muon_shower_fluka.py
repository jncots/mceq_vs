...
def muon_shower_data(meta_data_json):
    with open(meta_data_json, "r") as f:
        meta_data = json.load(f)

    data_dir = Path(meta_data["base_dir"])
    
    files = {}
    plabels = []
    for i, key in enumerate(meta_data["key_name"]):
        files[key] = meta_data["file_name"][i]
        plabels.append(meta_data["label"][i])
    ...
    for key in files.keys():
        ...
        for i in range(block_file.num_blocks()):
            ...
            xdepth = int(separate_number(depth_info))
